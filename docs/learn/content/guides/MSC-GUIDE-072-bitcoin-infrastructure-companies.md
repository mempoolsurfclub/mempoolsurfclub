...
Blockstream’s Esplora repository and API documentation describe an open-source blockchain explorer backend and HTTP interface. The mempool project publishes open-source explorer, mempool, and API software that can be self-hosted. These repositories establish available code, interfaces, and project instructions at the access date.
