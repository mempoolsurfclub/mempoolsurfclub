...
This guide was researched on July 26, 2026 against Bitcoin Core 31.1 at tag `v31.1`, commit `9be056a8a72b624dae9623b2f7bded92c2a21c91`. Commands, test directories, CI jobs, presets, and tools are pinned to that implementation release.
...
A mature test strategy asks which failures each layer can detect and which remain outside it.
...
Focused suites can be run directly through `build/bin/test_bitcoin` with Boost test-runner options. This makes a narrow failure easier to reproduce and debug.

A unit test often replaces or constructs surrounding state. That isolation is useful, but it can hide interactions that appear only when full processes, databases, sockets, or multiple nodes are involved.

### Functional and integration tests run real processes

Bitcoin Core’s Python functional tests live in `test/functional/`. They launch one or more Bitcoin Core node processes, communicate over RPC or the P2P test interface, manipulate regtest chains, and inspect externally visible behavior.

The suite can be run through:

```sh
build/test/functional/test_runner.py
```

An individual test can be selected by name, and `--extended` adds tests that are not in the normal set. The framework manages temporary data directories, ports, logs, process startup, and cleanup.

Functional tests are integration evidence because they combine binaries, process boundaries, RPC, P2P messages, chainstate, mempool, wallet, indexes, and storage. They are slower and more exposed to timing or platform variation than unit tests.

### Regtest is controlled chain state, not testnet

Most Bitcoin Core functional tests use regtest. Regtest is a private chain where the test controls block creation and peer topology. It supports fast deterministic scenarios such as mining blocks on demand, creating competing branches, triggering reorganizations, confirming and disconnecting transactions, testing wallet conflicts, and sending malformed P2P messages.
...
A regression test starts with a specific defect or unsafe behavior. The test should fail on the affected version, exercise the relevant boundary, and pass after the correction. This creates durable evidence that the known case remains fixed.
...
For security-sensitive defects, public regression coverage may be delayed until disclosure and upgrade considerations are handled. The absence of a public test does not mean no private reproduction occurred.
...
The current libFuzzer workflow uses CMake presets:
...
The project’s shared seed corpora are maintained in the `bitcoin-core/qa-assets` repository. Seed inputs help the fuzzer reach meaningful structures, while mutations explore nearby and combined cases. Coverage-increasing inputs can be preserved for future runs.

Fuzzing is not exhaustive. The input space is too large, some states are hard to reach, and a harness may omit important surrounding behavior. Millions of executions without failure provide evidence, not proof that every malformed input is safe.
...
Consensus-critical boundaries deserve special attention around integer ranges, lock times, sequence values, script limits, subsidy transitions, duplicate data, serialization lengths, and chainstate transitions. Boundary values and one-step-beyond values often reveal assumptions hidden by ordinary examples.

### Static analysis, warnings, sanitizers, lint, and formatting

Static analysis examines source or compiled structure without needing a particular runtime input. Compiler warnings can expose type conversions, unreachable code, missing cases, and suspicious constructs. Bitcoin Core documents clang-tidy workflows and uses repository-specific checks.

Sanitizers instrument a build to detect selected runtime defects. AddressSanitizer can detect many memory errors; UndefinedBehaviorSanitizer targets certain undefined operations; ThreadSanitizer looks for data races; MemorySanitizer tracks uninitialized reads under a fully instrumented environment. Each tool changes execution and has blind spots.

Bitcoin Core’s lint checks live under `test/lint/`. In 31.1, the project documents `./ci/lint.py` as the CI-parity local entry point. Lints cover source, documentation, shell, Python, tree consistency, generated data, and formatting-related rules. A formatting check can make review easier, but formatting is not correctness.
...
Tests can inject disconnects, mock time, run competing threads, restart nodes, corrupt or replace selected test files, and exercise graceful and abrupt shutdown paths. These tests can become flaky if they depend on real-time sleeps or assume scheduler order.
...
Chain tests cover block connection and disconnection, reorganizations, competing work, headers, pruning, indexes, assumeutxo background validation, and UTXO transitions. Header synchronization is not full block validation, and a test must identify which state it observes.

### Benchmarks measure performance, not correctness

Bitcoin Core’s `bench_bitcoin` framework measures selected operations. Bitcoin Core 31.1 can build it with `-DBUILD_BENCH=ON` and run focused benchmarks. Benchmarks can detect regressions and support performance decisions.
...
A green CI result means the configured jobs completed successfully for that commit. It does not mean every test ran, every platform was covered, external dependencies are safe, the code was reviewed, or production operations are ready. A missing job is not a passing job.

CI can also be wrong: caches can be stale, jobs can be skipped, infrastructure can fail, or a test can accidentally stop running. Reviewers should inspect which checks executed and what they actually covered.
...
When a test reveals a security-sensitive defect, public issue trackers may not be the appropriate first channel. Bitcoin Core maintains security reporting and disclosure processes so maintainers can reproduce the problem, assess affected releases, prepare fixes, and coordinate disclosure.
...
2. **Bitcoin Core 31.1 README** | Bitcoin Core contributors
   ...
   - Supports: Unit and functional test commands, cross-platform CI statement, independent QA, and code-review boundary.
3. **Bitcoin Core 31.1 Unit Test Documentation** | Bitcoin Core contributors
   ...
   - Supports: Unit-test source locations, CTest, `test_bitcoin`, focused suites, logs, temporary data, and debugger use.
4. **Bitcoin Core 31.1 Test Documentation** | Bitcoin Core contributors
   ...
   - Supports: Functional, fuzz, and lint test layers, current runner commands, caches, tracing, cleanup, and debugging.
5. **Bitcoin Core 31.1 Functional Test Documentation** | Bitcoin Core contributors
   ...
   - Supports: RPC and P2P testing, regtest framework, naming, coverage, cached chain, concurrency, and process-level test structure.
6. **Bitcoin Core 31.1 Functional Test Runner** | Bitcoin Core contributors
   ...
   - Supports: Current functional-test selection, parallel execution, extended tests, coverage, and result handling.
7. **Bitcoin Core 31.1 Functional Test Framework** | Bitcoin Core contributors
   ...
   - Supports: Node process control, regtest chains, RPC, P2P messages, block and transaction construction, and synchronization helpers.
8. **Bitcoin Core 31.1 Fuzzing Guide** | Bitcoin Core contributors
   ...
   - Supports: Current libFuzzer preset, target locations, corpora, sanitizer combinations, coverage inputs, and crash reproduction.
...
10. **Bitcoin Core QA Assets** | Bitcoin Core contributors
    ...
    - Supports: Shared fuzz seed corpora and reproducible corpus inputs used by Bitcoin Core testing.
...
12. **Bitcoin Core 31.1 Developer Notes** | Bitcoin Core contributors
    ...
    - Supports: clang-tidy, formatting, assertions, source conventions, static analysis, and review-oriented development practices.
13. **Bitcoin Core 31.1 CI Documentation** | Bitcoin Core contributors
    ...
    - Supports: CI scripts, varied configurations, dependencies, architecture emulation, sanitizer environment, caches, and local-run limitations.
...
15. **Bitcoin Core 31.1 Benchmarking Guide** | Bitcoin Core contributors
    ...
    - Supports: `bench_bitcoin` build and run commands, performance-regression purpose, environmental sensitivity, and distinction from fuzzing.
...
17. **Bitcoin Core 31.1 Script Test Data** | Bitcoin Core contributors
...
20. **Bitcoin Core 31.1 Mempool Functional Tests** | Bitcoin Core contributors
    - URL: https://github.com/bitcoin/bitcoin/tree/v31.1/test/functional
    - Supports: Current policy, replacement, package, reorganization, expiry, and mempool test evidence.
21. **Bitcoin Core Security Advisories** | Bitcoin Core project
    - URL: https://bitcoincore.org/en/security-advisories/
    - Supports: Public advisory context, affected-release communication, and security-sensitive disclosure boundary.
22. **Bitcoin Core 31.1 Release Notes** | Bitcoin Core contributors
    - URL: https://github.com/bitcoin/bitcoin/blob/v31.1/doc/release-notes.md
    - Supports: Current fixes across validation, P2P, wallet, tests, fuzzing, build, and CI, illustrating layered regression coverage.
...
- [x] Unit, functional, integration, regtest, test-vector, regression, fuzz, property, static-analysis, sanitizer, lint, benchmark, CI, and manual-review roles remain distinct.
- [x] Current Bitcoin Core 31.1 locations and commands are identified for unit tests, functional tests, fuzz targets, test data, linting, benchmarking, and CI.
- [x] Regtest is not confused with testnet, signet, or mainnet.
- [x] Consensus rules, policy rules, wallet behavior, RPC behavior, P2P behavior, and implementation choices remain distinct.
- [x] Network, timing, concurrency, database, index, shutdown, reorganization, serialization, and malformed-input testing are covered.
- [x] Wallet, RPC, P2P, mempool, policy, chainstate, and consensus-critical boundary tests are identified.
- [x] Fuzz corpora, seed management, coverage, determinism, flakiness, and regression reproduction are qualified.
- [x] Coverage percentages, benchmarks, passing CI, and reproduction of one case are not presented as production readiness or universal safety.
- [x] Test code, CI configuration, static tools, and review are acknowledged as fallible.
...
- [x] Planned internal links remain inactive, no publication is implied, and no correctness or security guarantee is made.
