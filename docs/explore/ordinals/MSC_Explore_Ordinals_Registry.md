...
Standards, protocols, indexing conventions, and experiments that use Ordinals inscriptions or ordinal-aware infrastructure as a substrate.

- Core Ordinals
- Fungible-token experiments
- Block-data / spatial standards
- Block-data / non-arbitrary assets
- Programmable inscription metaprotocols
- Recursive collection tooling
- Pre-Runes distribution experiments

### Collections & Projects

Canonical art, PFP, community, conceptual, generative, recursive, rare-sat, and distribution projects whose identity is materially tied to Ordinals.

- Early inscription art
- Early large collections
- PFP & community collections
- Generative / recursive / interactive art
- Conceptual / physical-digital art
- Conceptual / rare-sat experiments
- Cultural / meme lineage
- Airdrops / distribution experiments
- Bitcoin-history art

### Historical Cohorts & Indexes

Historically meaningful inscription-number cohorts and index concepts that are not single coordinated projects.

- Early inscription cohorts

## Complete inventory

### Protocols & Metaprotocols

- **Ordinal Theory & Inscriptions** — `ACTIVE` — `ordinal-theory-inscriptions` — Core Ordinals — Casey Rodarmor’s ordinal theory and inscription system for tracking individual satoshis and attaching digital artifacts to Bitcoin transactions.
- **BRC-20** — `ACTIVE` — `brc-20` — Fungible-token experiments — An experimental fungible-token convention introduced by the pseudonymous domo using Ordinals inscriptions for deploy, mint, and transfer operations.
- **Bitmap** — `ACTIVE` — `bitmap` — Block-data / spatial standards — A metaprotocol that lets users inscribe Bitcoin block-height identifiers such as `840000.bitmap` to claim abstract digital districts based on Bitcoin blocks.
- **Digital Matter Theory (DMT)** — `ACTIVE` — `digital-matter-theory` — Block-data / non-arbitrary assets — A framework for deriving digital assets and traits from patterns already present in Bitcoin block data rather than assigning arbitrary off-chain rarity.
- **Non-Arbitrary Tokens (NAT)** — `ACTIVE` — `non-arbitrary-tokens` — Block-data / non-arbitrary assets — Digital Matter Theory token conventions that derive supply or traits from deterministic Bitcoin block-data patterns.
- **TAP Protocol** — `ACTIVE` — `tap-protocol` — Programmable inscription metaprotocols — An Ordinals-based metaprotocol that extends BRC-20-style assets with additional transfer, trading, authority, DMT, and Bitmap-oriented capabilities.
- **GeneratOrd** — `UNCERTAIN` — `generatord` — Recursive collection tooling — A recursive/generative Ordinals framework created by the Fomojis team to compose collection assets from shared on-chain inscription resources.
- **RSIC Metaprotocol** — `HISTORICAL` — `rsic-metaprotocol` — Pre-Runes distribution experiments — A 21,000-piece Ordinals distribution experiment that gamified a pre-Runes 'mining' phase and later connected holders to RSIC•GENESIS•RUNE.

### Collections & Projects

- **Bitcoin Shrooms** — `INACTIVE` — `bitcoin-shrooms` — Early inscription art — Shroomtoshi’s pioneering Bitcoin-native pixel-art collection, recognized for very early inscriptions and strong use of Bitcoin cultural references.
- **Ordinal Maxi Biz** — `ACTIVE` — `ordinal-maxi-biz` — PFP & community collections — A prominent Ordinals portrait collection created by ZK Shark with art by Tony Tafuro, known for inscription provenance, sat hunting, and high-profile auction-house exposure.
- **Bitcoin Puppets** — `ACTIVE` — `bitcoin-puppets` — PFP & community collections — A 10,001-piece free-mint Ordinals collection by Le Puppeteer Fou whose deliberately absurd puppet aesthetic developed into a large community-driven culture.
- **O.P.I.U.M.** — `HISTORICAL` — `opium` — Cultural / meme lineage — A 777-piece 2023 free-mint precursor created by Le Puppeteer Fou before Bitcoin Puppets.
- **NodeMonkes** — `ACTIVE` — `nodemonkes` — Early large collections — A 10,000-piece original-art Ordinals PFP collection notable for completing inscription of its full 10k before other original-art 10k peers.
- **The Wizards of Ord** — `ACTIVE` — `wizards-of-ord` — Generative / recursive / interactive art — A Bitcoin-native art and tooling project led by lifofifo and Nicole, known for OrdRain, Nakamoto Archives, Block 78 work, recursion experiments, and Ordinals utilities.
...
- **Ordinal Punks** — `HISTORICAL` — `ordinal-punks` — Early inscription art — A 100-piece early Ordinals PFP set inspired by CryptoPunks, with all works inscribed within roughly the first 650 inscription numbers.
...
- **Sub-1k Inscriptions** — `HISTORICAL` — `sub-1k-inscriptions` — Early inscription cohorts — A collector-defined cohort of inscriptions with numbers below 1,000, valued primarily for extreme early placement in Ordinals history.
- **Sub-10k Inscriptions** — `HISTORICAL` — `sub-10k-inscriptions` — Early inscription cohorts — A collector-defined cohort covering the first roughly ten thousand numbered Ordinals inscriptions.
